<template>
  <div>
    ...
    <el-table
      ...
      v-loading="loading"
    >
      <el-table-column prop="book_name" label="书名" align="center" width="200"></el-table-column>
      <el-table-column prop="book_price" label="价格" align="center" width="200"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      ...
      this.$axios
        .post("/create/", JSON.stringify(this.createData))
        .then(res => {
          if (res.data.code === 200) {
            ...
            this.$message.error("can't read books databases")
          }
        });
    ...
    handleConfirm(arg) {
      if (
        this.updateData.name === this.oldData.name &&
        this.updateData.price === this.oldData.price
      ) {
      ...
      this[arg] = false;
      this.$axios
        .post("/update/", JSON.stringify(this.updateData))
        .then(res => {
          if (res.data.code === 200) {
        ...
        .then(res => {
          if (res.data.code === 200) {
            if (res.data.data && JSON.parse(res.data.data).length > 0) {
              ...
              this.$message.error(
                `can't search contains of '${this.search}' in database`
              )
            }
          } else {
